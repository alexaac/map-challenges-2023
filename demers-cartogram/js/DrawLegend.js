...
export const drawAreaLegend = (args) => {
  ...
  const data = args.data,
    ...
    areaScale = d3.scaleLinear().range([5, maxAreaSize]).domain([0, maxData]),
    legendTextLeftPad = 8,
    ...
    legendArea =
      args.typeOfArea === 'circle'
        ? legend
            .selectAll('.legend-area')
            .data(legendData)
            .enter()
            .append('circle')
    ...
    legendNumber =
      args.typeOfArea === 'circle'
        ? legend
            ...
            .text(
              (d, i) => d + (i == legendData.length - 1 ? ' ' + legendText : '')
            )
        : legend
            .selectAll('.legend-number')
            .data(legendData)
